  ...
  return (
    <div className='flex justify-start mt-2 sticky top-0 bg-white  z-50 shadow-xl '>
      ...
      <div onClick={toggleCart}
        ...
        <ShoppingCartIcon className='relative w-10  text-4xl  ' />
        <span className='absolute -top-3 -right-1 bg-orange-400 rounded-full p-1 h-7 w-7 text-center  '>{Object.keys(cart).length}</span>
      </div>

      <div ref={ref}
        className='sideCart absolute top-0 right-0 bg-blue-200 py-10 px-8  transform transition-transform z-50  translate-x-full hidden h-[100vh] w-85
      '>
